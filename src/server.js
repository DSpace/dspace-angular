...
export function startServer(bootstrap) {
    var app = express();
    ...
    function ngApp(req, res) {
        ...
        if (ENV_CONFIG.universal.preboot) {
            Zone.current.fork({ name: 'CSR fallback', onHandleError: onHandleError }).run(function () {
    ...
    function createHttpsServer(keys) {
        ...
        }, app).listen(ENV_CONFIG.ui.port, ENV_CONFIG.ui.host, function () {
            serverStarted();
        });
    }
    if (ENV_CONFIG.ui.ssl) {
        var serviceKey = void 0;
        try {
