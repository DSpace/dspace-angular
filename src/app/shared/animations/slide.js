...
export var slideHorizontal = trigger('slideHorizontal', [
    state('void', style({ width: 0 })),
    state('*', style({ width: '*' })),
    transition(':enter', [animate('200ms')]),
    transition(':leave', [animate('200ms')])
]);
export var slideMobileNav = trigger('slideMobileNav', [
    state('expanded', style({ height: '100vh' })),
    state('collapsed', style({ height: 0 })),
    transition('expanded <=> collapsed', animate('300ms'))
]);
var collapsedStyle = style({ marginLeft: '-{{ sidebarWidth }}' });
var expandedStyle = style({ marginLeft: '0' });
var options = { params: { sidebarWidth: '*' } };
export var slideSidebar = trigger('slideSidebar', [
...
export var slideSidebarPadding = trigger('slideSidebarPadding', [
