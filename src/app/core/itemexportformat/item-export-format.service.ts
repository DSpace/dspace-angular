...
/**
 * Enum representing the multiplicity of an item export operation.
 * SINGLE is used for exporting individual items, MULTIPLE for bulk exports.
 */
...
export class ItemExportFormatService extends IdentifiableDataService<ItemExportFormat> {
  ...
  /**
   * Launch a script via the ScriptDataService and return the process ID on success.
   * Shows an error notification on failure.
   * @param scriptName - The name of the script to invoke
   * @param parameterValues - The parameters to pass to the script
   * @returns Observable emitting the process ID on success, or null on failure
   */
