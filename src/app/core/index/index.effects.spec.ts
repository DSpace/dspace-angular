...
describe('ObjectUpdatesEffects', () => {
  ...
  describe('addAlternativeObjectLink$', () => {
    let action;
    it('should do nothing when the objectToCache has no uuid', () => {
      objectToCache = Object.assign(objectToCache, { uuid: undefined });
      action = new AddToObjectCacheAction(objectToCache, timeAdded, msToLive, requestUUID, alternativeLink);
    ...
    it('should emit NO_ACTION when a AddToObjectCacheAction without an alternativeLink is dispatched', () => {
      action = new AddToObjectCacheAction(objectToCache, timeAdded, msToLive, requestUUID, undefined);
      actions = hot('--a-', { a: action });
    ...
    it('should emit NO_ACTION when a AddToObjectCacheAction with an alternativeLink that\'s the same as the objectToCache\'s selfLink is dispatched', () => {
      action = new AddToObjectCacheAction(objectToCache, timeAdded, msToLive, requestUUID, objectToCache._links.self.href);
      actions = hot('--a-', { a: action });
      const expected = cold('--b-', { b: { type: 'NO_ACTION' } });
      expect(indexEffects.addAlternativeObjectLink$).toBeObservable(expected);
    });
