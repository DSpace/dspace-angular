...
export class ValuepairComponent extends RenderingTypeValueModelComponent implements OnInit {
  ...
  ngOnInit(): void {
    const entry$ = this.vocabularyService.getPublicVocabularyEntryByValue('common_iso_languages', this.metadataValue).pipe(
      getFirstSucceededRemoteDataPayload(),
      getPaginatedListPayload(),
      map((res) => res[0]?.display ?? this.metadataValue),
    );

    const initValue$ = interval(5000).pipe(mapTo(this.metadataValue));
