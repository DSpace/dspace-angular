...
export function serverApi(req, res) {
    ...
    var cache = fakeDemoRedisCache.get(key);
    if (cache !== undefined) {
...
export function createMockApi() {
    var router = Router();
    router.route('/communities').get(function (req, res) {
        console.log('GET');
        // 70ms latency
        setTimeout(function () {
    ...
    router.param('community_id', function (req, res, next, communityId) {
        // ensure correct prop type
        var id = req.params.community_id;
        try {
            req.community_id = id;
            req.community = COMMUNITIES.communities.find(function (community) {
    ...
    router.route('/communities/:community_id').get(function (req, res) {
    ...
    router.param('collection_id', function (req, res, next, collectionId) {
        // ensure correct prop type
        var id = req.params.collection_id;
        try {
            req.collection_id = id;
            req.collection = COLLECTIONS.collections.find(function (collection) {
    ...
    router.route('/collections/:collection_id').get(function (req, res) {
    ...
    router.param('item_id', function (req, res, next, itemId) {
        ...
        try {
            req.item_id = id;
            req.item = ITEMS.items.find(function (item) {
                return item.id === id;
            });
            next();
        }
    ...
    router.route('/bundles').get(function (req, res) {
        console.log('GET');
        // 70ms latency
        setTimeout(function () {
    ...
    router.param('bundle_id', function (req, res, next, bundleId) {
        // ensure correct prop type
        var id = req.params.bundle_id;
        try {
            req.bundle_id = id;
            req.bundle = BUNDLES.bundles.find(function (bundle) {
                return bundle.id === id;
            });
            next();
        }
    ...
    router.route('/bundles/:bundle_id').get(function (req, res) {
        // console.log('GET', util.inspect(req.bundle, { colors: true }));
        res.json(toHALResponse(req, req.bundle));
    });
    router.route('/bitstreams').get(function (req, res) {
        console.log('GET');
        // 70ms latency
        setTimeout(function () {
    ...
    router.param('bitstream_id', function (req, res, next, bitstreamId) {
        // ensure correct prop type
        var id = req.params.bitstream_id;
        try {
            req.bitstream_id = id;
            req.bitstream = BITSTREAMS.bitstreams.find(function (bitstream) {
    ...
    router.route('/bitstreams/:bitstream_id').get(function (req, res) {
