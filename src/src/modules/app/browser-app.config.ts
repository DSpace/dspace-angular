...
export const browserAppConfig: ApplicationConfig = mergeApplicationConfig({
  providers: [
    ...
    // Load 'en' translations before the page renders.
    // For SSR pages: translations are in TransferState → resolves instantly.
    // For CSR pages (excluded from SSR): fetches the translation JSON over HTTP so
    // translated text is available when the page first renders, not raw keys.
